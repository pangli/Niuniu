...
/**
 * Created by pangli on 2016/12/19.
 * 备注：
 */
...
public class ReadMoreTextView extends AppCompatTextView {
    ...
    private void initAttr(Context context, AttributeSet attrs) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (attr == R.styleable.ReadMoreTextView_rtv_MaxLinesOnShrink) {
                mMaxLinesOnShrink = a.getInteger(attr, MAX_LINES_ON_SHRINK);
            } else if (attr == R.styleable.ReadMoreTextView_rtv_EllipsisHint) {
                mEllipsisHint = a.getString(attr);
            } else if (attr == R.styleable.ReadMoreTextView_rtv_ToExpandHint) {
                mToExpandHint = a.getString(attr);
            } else if (attr == R.styleable.ReadMoreTextView_rtv_ToShrinkHint) {
                mToShrinkHint = a.getString(attr);
            } else if (attr == R.styleable.ReadMoreTextView_rtv_EnableToggle) {
                mToggleEnable = a.getBoolean(attr, TOGGLE_ENABLE);
            } else if (attr == R.styleable.ReadMoreTextView_rtv_ToExpandHintShow) {
                mShowToExpandHint = a.getBoolean(attr, SHOW_TO_EXPAND_HINT);
            } else if (attr == R.styleable.ReadMoreTextView_etv_ToShrinkHintShow) {
                mShowToShrinkHint = a.getBoolean(attr, SHOW_TO_SHRINK_HINT);
            } else if (attr == R.styleable.ReadMoreTextView_rtv_ToExpandHintColor) {
                mToExpandHintColor = a.getInteger(attr, TO_EXPAND_HINT_COLOR);
            } else if (attr == R.styleable.ReadMoreTextView_rtv_ToShrinkHintColor) {
                mToShrinkHintColor = a.getInteger(attr, TO_SHRINK_HINT_COLOR);
            } else if (attr == R.styleable.ReadMoreTextView_rtv_ToExpandHintColorBgPressed) {
                mToExpandHintColorBgPressed = a.getInteger(attr, TO_EXPAND_HINT_COLOR_BG_PRESSED);
            } else if (attr == R.styleable.ReadMoreTextView_rtv_ToShrinkHintColorBgPressed) {
                mToShrinkHintColorBgPressed = a.getInteger(attr, TO_SHRINK_HINT_COLOR_BG_PRESSED);
            } else if (attr == R.styleable.ReadMoreTextView_rtv_InitState) {
                mCurrState = a.getInteger(attr, STATE_SHRINK);
            } else if (attr == R.styleable.ReadMoreTextView_rtv_GapToExpandHint) {
                mGapToExpandHint = a.getString(attr);
            } else if (attr == R.styleable.ReadMoreTextView_rtv_GapToShrinkHint) {
    ...
    public interface OnExpandListener {
        void onExpand(ReadMoreTextView view);

        void onShrink(ReadMoreTextView view);
    }
    ...
    private class TouchableSpan extends ClickableSpan {
        ...
        public void onClick(View widget) {
            if (hasOnClickListeners()
                    && (getOnClickListener(ReadMoreTextView.this) instanceof ExpandableClickListener)) {
            } else {
                toggle();
            }
        }
